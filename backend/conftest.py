...
@pytest.fixture
def org_a():
    # Fixtures represent established (approved) orgs so org-scoped endpoints are
    # reachable. Pending-org behavior is exercised explicitly where needed.
    return Org.objects.create(name="Test Organization A", approval_status="approved")


@pytest.fixture
def org_b():
    return Org.objects.create(name="Test Organization B", approval_status="approved")
