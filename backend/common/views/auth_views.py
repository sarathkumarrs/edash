...
def _mark_pending_profiles_joined(user):
    """Stamp the joined date on a user's pending (invited) profiles.

    A Profile with ``date_of_joining=None`` is an outstanding invitation; the
    first time the invitee authenticates we record the day they joined.
    """
    Profile.objects.filter(user=user, date_of_joining__isnull=True).update(
...
class GoogleOAuthCallbackView(APIView):
    ...
    def post(self, request):
        ...
        # First login after an invite: stamp the joined date on pending profiles.
        _mark_pending_profiles_joined(user)
    ...
    def post(self, request):
        ...
        try:
            ...
            if org_id:
                ...
                # Don't keep refreshing org-context tokens for an org that is
                # no longer approved (e.g. approval revoked after issuance).
                if org.approval_status != Org.APPROVAL_APPROVED:
                    audit_log.token_revoked(
                        user, org, f"Org {org_id} not approved", request
                    )
                    return Response(
                        {"error": "This organization is pending superadmin approval."},
                        status=status.HTTP_403_FORBIDDEN,
                    )
...
class OrgSwitchView(APIView):
    ...
    def post(self, request):
        ...
        # Block switching into an org that is not yet approved by a superadmin.
        if profile.org.approval_status != Org.APPROVAL_APPROVED:
            audit_log.permission_denied(
                request.user, from_org, "ORG_SWITCH", f"org:{org_id}:pending", request
            )
            return Response(
                {"error": "This organization is pending superadmin approval."},
                status=status.HTTP_403_FORBIDDEN,
...
class MagicLinkVerifyView(APIView):
    ...
    def post(self, request):
        ...
        # Get user's organizations
        # Only auto-select an approved org as the login default; a user whose
        # only org is still pending logs in without org context (org locked).
        profiles = Profile.objects.filter(
            user=user, is_active=True, org__approval_status=Org.APPROVAL_APPROVED
        )
        default_org = None
        profile = None
