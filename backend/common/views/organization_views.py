...
class OrgProfileCreateView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            # Seed the default lead-interaction outcome list so the "Log
            # Contact" form has options out of the box (admin can edit them at
            # /settings/custom-fields). custom_field_definition is RLS-scoped,
            # so set the new org's context before inserting.
            from common.tasks import set_rls_context
            from leads.outcomes import seed_default_outcomes

            set_rls_context(org_obj.id)
            seed_default_outcomes(org_obj)

            # The org starts in "pending" state (model default) and is locked
            # until a superadmin approves it. Notify the superadmins by email.
            from common.tasks import notify_superadmin_new_org

            notify_superadmin_new_org.delay(str(org_obj.id), str(request.user.id))

            return Response(
                {
                    "error": False,
                    "message": (
                        "Organization created and is pending superadmin approval. "
