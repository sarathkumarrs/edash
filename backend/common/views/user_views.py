...
class UsersListView(APIView, LimitOffsetPagination):
    ...
    def post(self, request, format=None):
        """Invite a user to the current organization by email.

        Creates (or reuses) a passwordless user and a *pending* Profile
        (``date_of_joining=None``) and emails an invitation. The invitee
        becomes "joined" — date stamped — on their first login.
        """
        ...
        if not email:
            return Response(
                {"error": True, "errors": "Email is required"},
        ...
        try:
            ...
            return Response(
                {"error": True, "errors": "Enter a valid email address"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if role not in [r[0] for r in ROLES]:
            return Response(
                {"error": True, "errors": "Invalid role"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if Profile.objects.filter(user=user, org=org).exists():
            return Response(
        ...
        profile = Profile.objects.create(
            ...
            org=org,
            role=role,
            date_of_joining=None,  # pending until the invitee's first login
            is_active=True,
        )

        from common.tasks import send_org_invitation

        send_org_invitation.delay(str(profile.id), str(request.user.id))

        return Response(
            {"error": False, "message": "Invitation sent"},
