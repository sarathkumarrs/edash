...
class TestUserInvite:
    """Invite-by-email flow: pending profile + joined-on-first-login."""

    url = "/api/users/"

    def test_invite_creates_pending_profile_and_emails(self, admin_client, org_a):
        from unittest.mock import patch

        with patch("common.tasks.send_org_invitation.delay") as mock_invite:
            response = admin_client.post(
                self.url,
                {"email": "invitee@test.com", "role": "USER"},
                format="json",
            )
        assert response.status_code == status.HTTP_201_CREATED
        profile = Profile.objects.get(user__email="invitee@test.com", org=org_a)
        # Pending until first login.
        assert profile.date_of_joining is None
        assert profile.role == "USER"
        mock_invite.assert_called_once()

    def test_invite_duplicate_returns_400(self, admin_client, org_a, user_profile):
        # user_profile already puts regular_user in org_a.
        response = admin_client.post(
            self.url,
    ...
    def test_first_login_stamps_joined_date(self, org_a):
        from django.utils import timezone

        from common.models import User
        from common.views.auth_views import _mark_pending_profiles_joined

        user = User.objects.create_user(email="pendingjoin@test.com", password="x")
        profile = Profile.objects.create(
            user=user, org=org_a, role="USER", is_active=True, date_of_joining=None
        )
        _mark_pending_profiles_joined(user)
        profile.refresh_from_db()
        assert profile.date_of_joining == timezone.now().date()
