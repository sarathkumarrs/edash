...
#
# "LeadInteraction" is an exception: it has NO custom_fields column. It is
# registered here only so admins can define a `dropdown` definition that
# supplies the admin-managed option list for the typed `outcome` column on
# LeadInteraction (see leads.models.LeadInteraction). Never call
# validate_payload() for this target — its values live in typed columns.
