...
class LeadInteraction(BaseModel):
    """A single recorded touch with a lead (call, email, meeting, note).

    One row per contact event, forming a timeline on the lead. Logging an
    interaction stamps the lead's ``last_contacted`` (so it leaves the
    dashboard "New Leads to Contact" banner) and, when a follow-up date is
    provided, advances ``next_follow_up``.

    ``outcome`` is a free-form string validated at the API layer against the
    org's admin-managed option list — a ``dropdown`` CustomFieldDefinition with
    ``target_model="LeadInteraction"`` and ``key="outcome"``, editable at
    /settings/custom-fields. Storing it as a typed column (rather than in a
    custom_fields JSON blob) keeps it filterable/reportable.
    """
    ...
    interaction_type = models.CharField(
        _("Type"), max_length=16, choices=INTERACTION_TYPES, default="call"
    )
    outcome = models.CharField(_("Outcome"), max_length=64, blank=True, null=True)
    notes = models.TextField(_("Notes"))
    occurred_at = models.DateTimeField(_("Occurred At"), default=timezone.now)
    next_follow_up = models.DateField(_("Next Follow-up"), blank=True, null=True)

    org = models.ForeignKey(
        Org, on_delete=models.CASCADE, related_name="lead_interactions"
    )
    ...
    def __str__(self):
        return f"{self.get_interaction_type_display()} with {self.lead} on {self.occurred_at:%Y-%m-%d}"
