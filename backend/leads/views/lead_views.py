...
class LeadListView(APIView, LimitOffsetPagination):
    ...
    def get_context_data(self, **kwargs):
        ...
        if params:
            ...
            if params.get("outcome"):
                # Leads that have at least one logged interaction with this outcome.
                queryset = queryset.filter(
                    interactions__outcome=params.get("outcome")
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            # Phone uniqueness per org (no DB constraint on phone — enforce here
            # so the same number can't land on two different leads).
            ...
            if phone_raw:
                ...
                if normalized and any(
                    normalize_phone(p) == normalized
                    for p in Lead.objects.filter(org=request.profile.org)
                    ...
                    return Response(
                        {
                            ...
                            "errors": {
                                "phone": [
                                    "A lead with this phone number already exists in your organization."
                                ]
                            },
                        },
                        status=status.HTTP_400_BAD_REQUEST,
...
class LeadDetailView(APIView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_superuser or self.request.profile.role == "ADMIN":
            ...
            creator = self.lead_obj.created_by
            # NOTE: this project's User model has no `username` — it uses email.
            users_mention = [{"user__email": creator.email}] if creator else []
        else:
