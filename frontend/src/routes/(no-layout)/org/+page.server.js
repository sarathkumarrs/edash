  ...
  try {
    ...
    if (response.data.organizations && Array.isArray(response.data.organizations)) {
      orgs = response.data.organizations.map((org) => ({
        ...
        logo: org.logo || null,
        role: org.role || 'USER',
        approval_status: org.approval_status || 'approved',
        is_approved: org.is_approved ?? org.approval_status === 'approved'
      }));
    }
...
export const actions = {
  selectOrg: async ({ request, cookies }) => {
    ...
    try {
      ...
      // The org is still pending superadmin approval (or was rejected).
      if (error.response?.status === 403) {
        return fail(403, {
          error:
            error.response.data?.error ||
            'This organization is pending superadmin approval.'
        });
      }
