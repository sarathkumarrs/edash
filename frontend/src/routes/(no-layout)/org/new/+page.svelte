<script>
  ...
  import { Building2, ArrowLeft, Check, AlertCircle, Sparkles, Clock } from '@lucide/svelte';

  let { form } = $props();

  let isSubmitting = $state(false);

  // Handle form submission success - redirect after showing the pending message
  $effect(() => {
    if (form?.data) {
...
<svelte:head>
  <title>Create Organization | EdashCRM</title>
</svelte:head>
        ...
        >
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-[var(--text-secondary)]">
              This will be your workspace name in EdashCRM
            </p>
          </div>
          ...
          {#if form?.data}
            <div
              class="flex items-start gap-3 rounded-lg border border-[var(--color-warning-default)]/20 bg-[var(--color-warning-light)] p-4"
            >
              <Clock class="h-5 w-5 shrink-0 text-[var(--color-warning-default)]" />
              <div>
                <p class="text-sm font-medium text-[var(--color-warning-default)]">
                  Organization created — pending approval
                </p>
                <p class="mt-1 text-sm text-[var(--color-warning-default)]/80">
                  A superadmin needs to approve it before you can start using it. We’ll email you
                  once it’s approved.
                </p>
              </div>
            </div>
