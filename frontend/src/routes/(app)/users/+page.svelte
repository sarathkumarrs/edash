<script>
  ...
  let users = $derived(
    Array.isArray(data.users)
      ? data.users.map((u) => ({
          ...
          email: u.user.email,
          role: u.role,
          joined: u.profile?.date_of_joining
            ? typeof u.profile.date_of_joining === 'string'
              ? u.profile.date_of_joining.slice(0, 10)
              : new Date(u.profile.date_of_joining).toISOString().slice(0, 10)
            : '',
  ...
  $effect(() => {
    if (form?.success) {
      ...
      } else if (form.action === 'delete_user') {
  ...
  /**
   * Permanently delete a user / cancel a pending invitation
   * @param {string} userId
   */
...
<svelte:head>
  <title>Users & Teams - EdashCRM</title>
</svelte:head>
                    ...
                    {#each users as user (user.id)}
                      <Table.Row>
                        ...
                        <Table.Cell>
                          ...
                          {:else}
                            <span
                              class="inline-flex items-center gap-1 rounded-full border border-[var(--color-warning-default)]/20 bg-[var(--color-warning-light)] px-2 py-0.5 text-xs font-medium text-[var(--color-warning-default)]"
                            >Pending</span>
                          {/if}
                        </Table.Cell>
                        <Table.Cell>
                          ...
                          {:else if !user.isActive}
                            <!-- Inactive / pending user: Activate + permanently delete -->
                            <div class="flex items-center gap-1">
                              ...
                              <AlertDialog.Root>
                                <AlertDialog.Trigger
                                  ...
                                  title={user.joined ? 'Delete user' : 'Cancel invitation'}
                                >
                                ...
                                <AlertDialog.Content>
                                  <AlertDialog.Header>
                                    <AlertDialog.Title>
                                      {user.joined ? 'Delete User' : 'Cancel Invitation'}
                                    </AlertDialog.Title>
                                  ...
                                  <AlertDialog.Footer>
                                    ...
                                    >
                                      {user.joined ? 'Delete' : 'Cancel Invitation'}
                                    </Button>
                                  </AlertDialog.Footer>
                                </AlertDialog.Content>
                              </AlertDialog.Root>
