...
export const actions = {
   ...
   */
  delete_user: async ({ request, locals, cookies }) => {
    const org = locals.org;
    const user = locals.user;
    ...
    try {
      ...
      // Permanently delete via Django API (hard delete)
      // Django endpoint: DELETE /api/user/{id}/
      await apiRequest(
        `/user/${user_id}/`,
        {
          method: 'DELETE'
      ...
      return { success: true, action: 'delete_user' };
    } catch (err) {
