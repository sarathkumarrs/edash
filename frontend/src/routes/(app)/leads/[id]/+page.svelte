<script>
  ...
  /** @type {{ data: { lead: any, comments: any[], attachments: any[], tags: any[], users: any[], commentPermission: boolean, customFieldDefinitions: any[], customFieldValues: Record<string, unknown>, interactions?: any[], outcomeOptions?: any[], user?: any } }} */
  let { data } = $props();

  const lead = $derived(data.lead || {});
  // Deals module may be disabled for this workspace — hide deal/convert UI.
  // (enabled_modules is merged in from the (app) layout load.)
  const showDeals = $derived(
    isModuleEnabled(/** @type {any} */ (data).enabled_modules, 'deals')
  );
  ...
  // Interaction timeline (logged contacts) + admin-managed outcome options.
  const interactions = $derived(data.interactions || []);
  const outcomeOptions = $derived(data.outcomeOptions || []);
  const currentUserId = $derived(/** @type {any} */ (data).user?.id);
  ...
  // Deep link from the dashboard "New Leads to Contact" banner: /leads/<id>?log=1
  // opens the Log Contact dialog straight away.
  $effect(() => {
    if ($page.url.searchParams.get('log') === '1') logOpen = true;
  });
...
>
  ...
  {#snippet actions()}
    <div class="flex items-center gap-1.5">
      ...
      <Button variant="default" size="sm" onclick={() => (logOpen = true)}>
        <PhoneCall class="mr-1.5 size-3.5" /> Log Contact
      </Button>
      <Button
        variant="outline"
        size="sm"
  ...
  <Tabs.Content class="" value="overview">
    <div class="grid grid-cols-1 gap-6 pt-4 pb-8 lg:grid-cols-[1fr_320px]">
      ...
      <div class="flex flex-col gap-6">
        ...
        <!-- Interactions (logged contacts) -->
        <SectionCard title="Interactions" class="px-4 py-2">
          {#if interactions.length === 0}
            <div class="flex flex-col items-center gap-2 py-6 text-center">
              <PhoneCall class="size-6 text-[color:var(--text-subtle)]" />
              <p class="text-sm text-[color:var(--text-muted)]">No contact logged yet</p>
                ...
                <li class="flex items-start gap-3 py-3">
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                      <strong class="text-sm text-[color:var(--text-default)]">{meta.label}</strong>
                      {#if it.outcome}
                        <Badge variant="secondary" class="text-[10px]">
                          {outcomeLabels[it.outcome] || it.outcome}
                        </Badge>
                      {/if}
                      <span class="text-xs text-[color:var(--text-subtle)]">
                        {it.occurred_at ? formatRelativeDate(it.occurred_at) : ''}
                      </span>
                      {#if it.created_by?.id === currentUserId}
                        <form
                          method="POST"
                          ...
                          use:enhance={() => async ({ result }) => {
                            if (result.type === 'success') {
                              ...
                              toast.error(
                                /** @type {any} */ (result.data)?.error || 'Failed to delete'
                              );
                            }
                          }}
                        >
                          ...
                          <button
                            type="submit"
                            class="text-[color:var(--text-subtle)] hover:text-[color:var(--color-negative-default)]"
                    ...
                    <p class="mt-1 text-sm whitespace-pre-wrap text-[color:var(--text-muted)]">
                      {it.notes}
                    </p>
                    <div class="mt-1 flex flex-wrap items-center gap-3 text-xs text-[color:var(--text-subtle)]">
      ...
      <div class="flex flex-col gap-6">
        ...
        <!-- Dates: always shown so reps see contact status at a glance -->
        <SectionCard title="Dates">
            <dl class="grid grid-cols-1 gap-y-3 text-[12px]">
              <div class="flex items-baseline justify-between gap-3">
                ...
                {:else}
                  <dd class="text-right text-[color:var(--text-subtle)] italic">Not contacted yet</dd>
                {/if}
              </div>
              <div class="flex items-baseline justify-between gap-3">
                ...
                {:else}
                  <dd class="text-right text-[color:var(--text-subtle)] italic">No follow-up set</dd>
                {/if}
              </div>
            </dl>
          </SectionCard>
...
<LogContactDialog bind:open={logOpen} leadId={lead?.id} {outcomeOptions} />
