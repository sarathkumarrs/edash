...
/**
 * Forward a single-file multipart upload to a Django endpoint, preserving auth.
 * @param {string} endpoint
 * @param {File} file
 * @param {import('@sveltejs/kit').Cookies} cookies
 */
async function forwardMultipart(endpoint, file, cookies, extra = {}) {
...
export async function load({ url, cookies, locals }) {
  ...
  try {
    ...
    if (filters.outcome) kanbanQueryParams.append('outcome', filters.outcome);
    ...
    const [
      ...
      // Admin-managed outcome options for the Log Contact dialog.
      apiRequest(
    ...
    const outcomeDef = (outcomesResponse?.definitions || []).find((d) => d.key === 'outcome');
...
export const actions = {
  ...
  getLead: async ({ request, locals, cookies }) => {
    try {
      ...
      const [response, interactionsRes] = await Promise.all([
        apiRequest(`/leads/${leadId}/`, { method: 'GET' }, { cookies, org: locals.org }),
        apiRequest(
  ...
  /**
   * Log a contact from the drawer quick-action bar.
   */
  logContact: async ({ request, locals, cookies }) => {
    const form = await request.formData();
    const leadId = form.get('leadId')?.toString();
    const notes = form.get('notes')?.toString().trim() || '';
    if (!leadId) return fail(400, { error: 'Lead ID is required.' });
  ...
  importLeadsPreview: async ({ request, cookies }) => {
    const form = await request.formData();
    const file = form.get('file');
    if (!(file instanceof File) || file.size === 0) {
      return fail(400, { importError: 'Please choose a CSV file.' });
    }
    const skip = form.get('skip_duplicates') === 'true';
    const { status, body } = await forwardMultipart('/leads/import/preview/', file, cookies, {
  ...
  importLeadsCommit: async ({ request, cookies }) => {
    const form = await request.formData();
    const file = form.get('file');
    if (!(file instanceof File) || file.size === 0) {
      return fail(400, { importError: 'Please choose a CSV file.' });
    ...
    const skip = form.get('skip_duplicates') === 'true';
    const { status, body } = await forwardMultipart('/leads/import/commit/', file, cookies, {
      skip_duplicates: skip
    });
    if (status !== 200) {
      return fail(status, {
