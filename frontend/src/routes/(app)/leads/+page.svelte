<script>
  ...
  import LogContactDialog from '$lib/components/leads/LogContactDialog.svelte';
  ...
  const drawerColumns = $derived([
    ...
    // Activity — shown in the core set when viewing an existing lead, but
    // hidden on the creation drawer (no contact has happened yet).
    {
      key: 'lastContacted',
      label: 'Last Contact',
      type: 'date',
  ...
  // Log Contact dialog (opened from the drawer quick-action bar).
  let logOpen = $state(false);
  // CSV import drawer.
  let importOpen = $state(false);
  const outcomeOptions = $derived(data.outcomeOptions || []);

  // Interactions shown in the drawer's Interactions tab (managed by CrmDrawer).
  const drawerInteractions = $derived(drawerData?.interactions || []);
  const outcomeLabelMap = $derived(
    Object.fromEntries(outcomeOptions.map((/** @type {any} */ o) => [o.value, o.label]))
  );
  const interactionTypeMeta = /** @type {Record<string, { label: string, icon: any }>} */ ({
  ...
  $effect(() => {
    ...
    if (action === 'create') {
      ...
      // Skip if the drawer is already showing this lead — otherwise a list
      // reload (e.g. invalidateAll after logging a contact) would overwrite the
      // enriched drawerData (interactions/comments) with the bare list row.
      if (drawerOpen && drawerData?.id === viewId) return;
  ...
  async function openLead(lead, fromKanban = false) {
    ...
    if (fromKanban) {
      ...
      // Table view: the list row already has the full field set, so render it
      // immediately, then enrich with interactions/comments (which the list
      // doesn't carry) via the server action.
      drawerData = lead;
  ...
  /**
   * Re-fetch the open lead's full data (interactions, comments) and update the
   * drawer in place. Used after opening from the table and after logging a
   * contact so the Interactions tab reflects the new entry without a reload.
   * @param {string} [leadId]
   */
  async function refreshDrawerLead(leadId) {
    const id = leadId || drawerData?.id;
    if (!id) return;
    try {
      const formData = new FormData();
      formData.append('leadId', id);
      const response = await fetch('?/getLead', { method: 'POST', body: formData });
      const result = deserialize(await response.text());
      if (result.type === 'success' && result.data?.lead && drawerData?.id === id) {
        drawerData = result.data.lead;
      }
    } catch (err) {
      console.error('Failed to refresh lead interactions/comments:', err);
    }
  }
    ...
    <FilterStrip>
      ...
      {#if outcomeOptions.length > 0}
        <SelectFilter
          ...
          onchange={(value) => updateFilters({ ...filters, outcome: value })}
          class="w-40"
        />
      {/if}
      <DateRangeFilter
...
<CrmDrawer
  ...
  onClose={closeDrawer}
  interactionsCount={drawerInteractions.length}
>
  {#snippet banner()}
    {#if drawerLoadError}
      <div
  ...
  {#snippet actionBar()}
    <div class="flex flex-wrap items-center gap-2">
      <Button variant="default" size="sm" class="gap-1.5" onclick={() => (logOpen = true)}>
        <PhoneCall class="size-3.5" /> Log Contact
      </Button>
      {#if drawerData?.phone}
        <Button
          variant="outline"
          ...
          class="size-8"
          title="WhatsApp"
          href={`https://wa.me/${String(drawerData.phone).replace(/\D/g, '')}`}
          target="_blank"
          rel="noopener"
        >
  ...
  {#snippet interactionsSection()}
    {#if drawerInteractions.length === 0}
      <div class="flex flex-col items-center gap-2 py-8 text-center">
        <PhoneCall class="size-6 text-[color:var(--text-subtle)]" />
        <p class="text-[13px] text-[color:var(--text-muted)]">No interactions logged yet</p>
        <Button variant="outline" size="sm" onclick={() => (logOpen = true)}>
          <PhoneCall class="mr-1.5 size-3.5" /> Log first contact
        </Button>
      </div>
    {:else}
      <ul class="divide-y divide-[color:var(--border-subtle)]">
        {#each drawerInteractions as it (it.id)}
          {@const meta = interactionTypeMeta[it.interaction_type] || interactionTypeMeta.note}
          <li class="flex items-start gap-3 py-3">
            <div
              class="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-full bg-[color:var(--bg-elevated)] text-[color:var(--text-muted)]"
            >
              <meta.icon class="size-3.5" />
            </div>
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                <strong class="text-[13px] text-[color:var(--text)]">{meta.label}</strong>
                {#if it.outcome}
                  <span
                    class="rounded-full bg-[color:var(--bg-elevated)] px-2 py-0.5 text-[10px] text-[color:var(--text-muted)]"
                  >
                    {outcomeLabelMap[it.outcome] || it.outcome}
                  </span>
                {/if}
                <span class="text-[11px] text-[color:var(--text-subtle)]">
                  {it.occurred_at ? formatRelativeDate(it.occurred_at) : ''}
                </span>
              </div>
              <p class="mt-1 text-[13px] whitespace-pre-wrap text-[color:var(--text-muted)]">
                {it.notes}
              </p>
              <div
                class="mt-1 flex flex-wrap items-center gap-3 text-[11px] text-[color:var(--text-subtle)]"
              >
                {#if it.created_by?.email}
                  <span>{it.created_by.name || it.created_by.email}</span>
                {/if}
                {#if it.next_follow_up}
                  <span class="inline-flex items-center gap-1">
                    <Calendar class="size-3" /> Follow up {formatDate(it.next_follow_up)}
                  </span>
                {/if}
              </div>
            </div>
...
<LogContactDialog
  bind:open={logOpen}
  leadId={drawerData?.id}
  {outcomeOptions}
  onLogged={() => refreshDrawerLead()}
/>

<LeadImportDrawer bind:open={importOpen} />
