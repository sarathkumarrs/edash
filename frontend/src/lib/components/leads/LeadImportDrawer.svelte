<script>
  ...
  /**
   * Two-step CSV import flow for leads: upload → preview → confirm.
   * Network calls go to the SvelteKit form actions on leads/+page.server.js,
  ...
  // When on, duplicate rows (email/phone) are skipped instead of blocking the import.
  let skipDuplicates = $state(true);

  /** @type {{ summary: { total: number, valid: number, invalid: number, skipped: number }, valid: any[], errors: { row: number, field: string, message: string }[], header_error: string | null } | null} */
  let preview = $state(null);
  /** @type {string | null} */
  let formError = $state(null);
  /** @type {{ row: number, field: string, message: string }[]} */
  let commitErrors = $state([]);
  /** @type {{ created: number, ids: string[], skipped?: number } | null} */
  ...
  function downloadErrorsList(errs) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'leads-import-errors.csv';
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  async function submitPreview() {
    ...
    try {
      ...
      const res = await fetch('?/importLeadsPreview', { method: 'POST', body: fd });
  ...
  async function submitCommit() {
    ...
    try {
      ...
      fd.append('file', file);
      fd.append('skip_duplicates', String(skipDuplicates));
      const res = await fetch('?/importLeadsCommit', { method: 'POST', body: fd });
      const result = /** @type {any} */ (deserialize(await res.text()));
      if (result.type === 'failure') {
        formError = String(result.data?.importError || 'Import failed');
        const errs = result.data?.importErrors;
        if (Array.isArray(errs) && errs.length) commitErrors = errs;
        return;
      }
  ...
  <Dialog.Content class="sm:max-w-3xl">
    <Dialog.Header>
      <Dialog.Title class="flex items-center gap-2">
        ...
        Import leads (CSV or Excel)
      </Dialog.Title>
      <Dialog.Description>
        Upload a CSV or Excel (.xlsx) file — e.g. a Meta Lead Ads export. We'll validate
        every row and flag duplicates by email before writing anything.
      </Dialog.Description>
    </Dialog.Header>
    ...
    {#if step === 'select'}
      <div class="space-y-4">
        <button
          ...
          ondrop={onDrop}
          onclick={() => document.getElementById('lead-import-file')?.click()}
        >
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
              ...
              {:else}
                <p class="text-sm font-medium">Drop a CSV or Excel file here, or click to choose</p>
                <p class="text-xs text-[var(--text-secondary)]">.csv or .xlsx, up to 5 MB</p>
              {/if}
            </div>
          </div>
        </button>
        ...
        >
          ...
          <p class="mt-1 text-[var(--text-secondary)]">
            Each row needs at least a name, email, or phone. Columns: first_name, last_name
            (or a single <code class="rounded bg-[var(--surface-default)] px-1">full_name</code>),
            email, <code class="rounded bg-[var(--surface-default)] px-1">phone</code>
            (or <code class="rounded bg-[var(--surface-default)] px-1">phone_number</code>), company,
            title, job_title, website, linkedin_url, source, status, rating (Hot/Warm/Cold),
            description, address_line, city, state, postcode, country (2-letter), assigned_emails,
            team_names, tags (semicolon-separated for the last three).
          </p>
          <p class="mt-1 text-[var(--text-secondary)]">
            Meta Lead Ads exports work directly — <code class="rounded bg-[var(--surface-default)] px-1">full_name</code>
            and <code class="rounded bg-[var(--surface-default)] px-1">phone_number</code> are recognized.
            Rows whose email already exists in your org (or earlier in the file) are flagged.
          </p>
          <div class="mt-2 flex flex-wrap items-center gap-4">
            <button
              type="button"
              class="inline-flex items-center gap-1 text-[var(--color-primary-default)] hover:underline"
            ...
            <a
              ...
              class="inline-flex items-center gap-1 text-[var(--color-primary-default)] hover:underline"
            >
              <Download class="h-3.5 w-3.5" />Download Excel template
            </a>
          </div>
        </div>
        ...
        <label class="flex items-start gap-2 text-sm">
          <input
            ...
            class="mt-0.5 h-4 w-4 rounded border-[var(--border-default)]"
          />
          <span>
            <span class="font-medium">Skip duplicate rows</span>
            <span class="block text-xs text-[var(--text-secondary)]">
              Rows whose email or phone already exists (in your org or earlier in the file)
              are skipped instead of blocking the import. Uncheck to treat duplicates as errors.
            </span>
          </span>
        </label>
    ...
    {:else if step === 'preview' && preview}
      ...
      <div class="space-y-4">
        <div class="flex flex-wrap items-center gap-2">
          ...
          {#if preview.summary.skipped > 0}
            <span
              class="inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-xs font-medium text-amber-800"
            >
              Skipped (duplicates): {preview.summary.skipped}
            </span>
          {/if}
        </div>
      ...
      <Dialog.Footer>
        ...
        >
          {#if busy}<Loader2 class="mr-1 h-3.5 w-3.5 animate-spin" />{/if}
          Import {preview.summary.valid} lead{preview.summary.valid === 1 ? '' : 's'}
        </Button>
      </Dialog.Footer>
        ...
        <div class="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4">
          ...
          <div>
            <p class="text-sm font-medium text-green-900">
              Imported {commitResult.created} lead{commitResult.created === 1 ? '' : 's'}
              {#if commitResult.skipped}
                · skipped {commitResult.skipped} duplicate{commitResult.skipped === 1 ? '' : 's'}
              {/if}
            </p>
            <p class="text-xs text-green-800">The leads list has been refreshed.</p>
          </div>
        </div>
      </div>
      <Dialog.Footer>
