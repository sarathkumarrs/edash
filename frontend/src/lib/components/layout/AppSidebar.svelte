<script>
  ...
  /** @type {Props} */
  let { user = {}, org_name = 'EdashCRM', org_settings = {}, role = 'USER' } = $props();
  ...
  const workspaceItems = $derived([
    ...
    ...(isAdmin
      ...
      : []),
    { href: '/leads', label: 'Pipeline', icon: Activity, type: 'link', preload: 'off', count: undefined, module: 'leads' }
  ]);

  const recordsItems = [
    { href: '/leads', label: 'Leads', icon: Target, type: 'link', preload: 'off', count: undefined, module: 'leads' },
    { href: '/contacts', label: 'Contacts', icon: Users, type: 'link', preload: 'off', count: undefined, module: 'contacts' },
    { href: '/accounts', label: 'Accounts', icon: Building, type: 'link', preload: 'off', count: undefined, module: 'accounts' },
    { href: '/opportunities', label: 'Deals', icon: Sparkles, type: 'link', preload: 'off', count: undefined, module: 'deals' }
  ];
  ...
  const workItems = [
    ...
    { href: '/tasks', label: 'Tasks', icon: CheckSquare, type: 'link', preload: 'off', count: undefined, module: 'tasks' },
    { href: '/timesheet', label: 'Timesheet', icon: Clock, type: 'link', preload: 'off', count: undefined, module: 'timesheet' },
    { href: '/goals', label: 'Goals', icon: Trophy, type: 'link', preload: 'off', count: undefined, module: 'goals' }
  ];
  ...
  const supportItems = [
    { href: '/support', label: 'Help desk', icon: HelpCircle, type: 'link', preload: 'off', count: undefined, module: 'helpdesk' }
  ];

  // Filter nav by the org's enabled modules (from the (app) layout data).
  // Items without a `module` (e.g. Home) are always shown.
  let enabledModules = $derived($page.data?.enabled_modules ?? null);
  const visibleItems = (items) =>
    items.filter((it) => !it.module || isModuleEnabled(enabledModules, it.module));
  let workspaceNav = $derived(visibleItems(workspaceItems));
  let recordsNav = $derived(visibleItems(recordsItems));
  let workNav = $derived(visibleItems(workItems));
  let revenueNav = $derived(visibleItems(revenueItems));
  let supportNav = $derived(visibleItems(supportItems));
