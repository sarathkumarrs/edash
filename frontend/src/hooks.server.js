...
import { ALL_MODULE_KEYS, isModuleEnabled, moduleForPath } from '$lib/modules';
 ...
 * @typedef {{ org_id?: string, org_name?: string, role?: string, user_id?: string, user_name?: string, user_email?: string, user_profile_pic?: string, exp?: number, iat?: number, org_settings?: OrgSettingsPayload, enabled_modules?: string[] }} JWTPayload
...
export const handle = sequence(Sentry.sentryHandle(), async function _handle({ event, resolve }) {
  ...
  if (jwtPayload) {
    ...
    if (orgId && !UUID_RE.test(orgId)) {
      ...
      if (tokenHasOrgContext(token, orgId)) {
        ...
        // Which modules this workspace shows (older tokens lack the claim -> all on)
        event.locals.enabled_modules = jwtPayload.enabled_modules || ALL_MODULE_KEYS;
      } else {
        ...
        if (switchResult) {
          ...
          event.locals.enabled_modules = newPayload?.enabled_modules || ALL_MODULE_KEYS;
        } else {
  ...
  if (isAuthOnlyRoute) {
    ...
    // Module visibility: a disabled module's routes are not reachable.
    const moduleKey = moduleForPath(pathname);
    if (moduleKey && !isModuleEnabled(event.locals.enabled_modules, moduleKey)) {
      throw redirect(303, '/');
    }
  }
